            ...
            <DropdownMenu.Root>
                ...
                <DropdownMenu.Portal>
                    <DropdownMenu.Content className="bg-zinc-800 p-2 rounded-lg mt-1 border-2 border-zinc-700" align="start">
                        <DropdownMenu.Item onSelect={() => selectedFace = "train"} className="h-9 flex items-center justify-between min-w-50">
                            Train: {faces["train"] || 0}
                        </DropdownMenu.Item>
                        ...
                        <DropdownMenu.Label className="text-zinc-400 mb-2 text-sm">Collections</DropdownMenu.Label>
                        {
                            Object.entries(faces).map(([name, count]) => (
                                name !== "train" && (
                                    <DropdownMenu.Item onSelect={() => selectedFace = name} key={name} className="h-9 flex items-center justify-between min-w-50">
