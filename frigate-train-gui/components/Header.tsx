    ...
    return (
        <header className="sticky top-0 z-10 bg-zinc-900">
            <DropdownMenu.Root>
                ...
                <DropdownMenu.Portal>
                    <DropdownMenu.Content className="bg-zinc-800 p-2 rounded-lg mt-1 border-2 border-zinc-700 z-50" align="start">
                        <DropdownMenu.Item onSelect={() => setSelectedFace("train")} className="h-9 flex items-center justify-between min-w-50 clickable">
                            Train: {faces["train"] || 0}
                        </DropdownMenu.Item>
                        ...
                        <DropdownMenu.Label className="text-zinc-400 mb-2 text-sm">Collections</DropdownMenu.Label>
                        {
                            Object.entries(faces).map(([name, count]) => (
                                name !== "train" && (
                                    <DropdownMenu.Item onSelect={() => setSelectedFace(name)} key={name} className="h-9 flex items-center justify-between min-w-50 clickable">
            ...
            <DropdownMenu.Root >
                ...
                <DropdownMenu.Portal>
                    <DropdownMenu.Content className="clickable p-2 rounded-lg mt-1 border-2 border-zinc-700 z-50" align="start">
